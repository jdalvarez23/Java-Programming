    ...
    public static void main(String[] args) {
        ...
        while (numberCounter <= limit) {
            ...
            if (number > largest) { // if the number inserted is larger
                secondLargest = largest; // set the second largest variable to the current largest value
                largest = number; // set largest variable to new number
            } else if ((number < largest) && (number > secondLargest)) { // if the number is smaller than the largest but greater than the second largest
                secondLargest = number; // set the second largest variable to the new number
            }
        ...
        if (numberCounter != 0) { // if user quits
            ...
            System.out.printf("Second largest integer inserted: %d%n", secondLargest);
